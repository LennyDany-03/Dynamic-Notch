...
description: >
  ...
  graph-paper grid, Geist serif 400 + Geist + Geist Mono, the top/bottom cobalt
...
colors:
  ...
  ink: "#F5F5F7"
  ink-soft: "#2F6FED"
  grid: "rgba(47, 111, 237, 0.10)"
  ink-faint: "rgba(47, 111, 237, 0.18)"

typography:
  # — reading ramp —
  body:        { fontFamily: "Geist", cqw: 0.83, weight: 400, lineHeight: 1.5 }
  body-lede:   { fontFamily: "Geist", cqw: 0.95, weight: 400, lineHeight: 1.5 }
  micro:       { fontFamily: "Geist", px: 13, weight: 600, tracking: "0.16em", upper: true }
  micro-strong:{ fontFamily: "Geist", px: 16, weight: 600, tracking: "0.18em", upper: true }
  mono-tag:    { fontFamily: "Geist Mono", cqw: 0.78, weight: 400, tracking: "0.05em" }
  mono-chrome: { fontFamily: "Geist Mono", px: 13, weight: 400, tracking: "0.06em" }
  # — display / hero ramp (Geist 400, negative tracking) —
  table-name:  { fontFamily: "Geist", cqw: 1.5, weight: 400, lineHeight: 1.15 }
  row-headline:{ fontFamily: "Geist", cqw: 2.1, weight: 400, lineHeight: 1.05 }
  ed-callout:  { fontFamily: "Geist", cqw: 2.6, weight: 400, lineHeight: 1.1, italic: true }
  headline:    { fontFamily: "Geist", cqw: 4.6, weight: 400, lineHeight: 0.95 }
  headline-index:{ fontFamily: "Geist", cqw: 5.0, weight: 400, lineHeight: 0.95 }
  display-quote:{ fontFamily: "Geist", cqw: 5.7, weight: 400, lineHeight: 1.05, tracking: "-0.005em" }
  display-chapter:{ fontFamily: "Geist", cqw: 6.8, weight: 400, lineHeight: 1.0, tracking: "-0.005em" }
  display-closing:{ fontFamily: "Geist", cqw: 9.4, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-hero:{ fontFamily: "Geist", cqw: 10.4, weight: 400, lineHeight: 0.9, tracking: "-0.008em" }
  vbig-numeral:{ fontFamily: "Geist", cqw: 12.5, weight: 400, lineHeight: 0.9, tracking: "-0.015em" }
...
components:
  ...
  ledger-row:
    layout: "grid: mono num · Geist name · Hanken desc · mono delta"
...
- **Weights** — the brand font ships `{400, 500, 550, 600, 700}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is a three-face conversation: **Geist** serif at weight 400 (size, not weight,
makes hierarchy) carries every display and headline; **Geist** carries body and
uppercase tracked labels; **Geist Mono** carries all chrome — page numbers, tags, ticks, vertical
...
- **Geist 400** for all display (negative tracking); **Hanken 600** uppercase 0.16em labels; **Geist Mono** chrome.
...
- **Squint** — one Geist element dominates at **3–6× its nearest neighbor** (`display-hero`/`vbig-numeral`); hierarchy is size, never weight.
...
Hanken→Geist, from a mono delta arrow, or from opacity, never from color.

## Typography

Two ramps. The **reading ramp** (Hanken body 0.83cqw, mono chrome in px) carries copy + labels;
the **display/hero ramp** (Geist `headline` 4.6cqw → `vbig-numeral` 12.5cqw) carries every
...
- **Geist at 400 only**, in cobalt, with negative tracking (hero −0.008em, numeral −0.015em). **Hanken labels uppercase, 0.16–0.18em.** **Geist Mono 0.04–0.08em.** No bold serif.
...
**Ground** paper + grid, hairlines. **Composes** pixel-glitch (right, ~26cqw), qr-block (top-right), display-hero, ed-callout. **Focal** a 1–2 line `display-hero` Geist headline in cobalt, left-anchored, with an italic `ed-callout` subtitle in `{colors.ink-soft}`. **Chrome** Hanken kicker; mono meta top + page number. **Accent** none (cobalt is the only ink). **Silence** ~45% paper. **Fixed** Geist 400, glitch + QR present, grid + hairlines. **Free** title, glitch step pattern, QR placement. **Density** sparse.

### 2 · Index Ledger (catalog · move: dense matrix · left — the dense frame)

**Ground** paper + grid, hairlines, `pad-top`/`pad-bottom`. **Composes** topbar-rule, ledger-rows. **Focal** a topbar (`headline-index` + mono lab-tag, 1.5px rule) over 4–6 ledger rows (mono num · Geist name · Hanken desc), ink-faint dividers. **Chrome** page number. **Accent** mono delta arrows. **Silence** tight — the density exception (the grid wants filling). **Fixed** 1.5px topbar rule, 1px ink-faint dividers, Geist names. **Free** rows, lab-tag, deltas. **Density** dense-exception.

### 3 · Chapter Opener (section · move: scale · sparse · left)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left, ~14cqw, low opacity), mono index, display-chapter, body-lede. **Focal** a `display-chapter` Geist title, with a small mono `CHAPTER NN` index above and an optional Hanken lede ≤42cqw. **Accent** none. **Silence** ~60% — let the grid breathe. **Fixed** Geist 400, glitch low-opacity, grid showing. **Free** title, lede, glitch side. **Density** sparse.
...
**Ground** paper + grid, hairlines. **Composes** display-quote (or display-manifesto), attribution rule, optional compact glitch. **Focal** a 2–3 line Geist pull in cobalt, with a Hanken kicker above and a 1px cobalt attribution rule + mono byline beneath. **Accent** none. **Silence** ~55% — deliberately open. **Fixed** Geist 400, attribution rule, grid showing. **Free** quote, byline. **Density** sparse.

### 6 · Colophon (closer · move: right-aligned close · sparse)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left edge, mirroring the cover), display-closing, mono credit columns. **Focal** a right-aligned `display-closing` Geist title with a Hanken kicker above. **Chrome** 3–4 column mono credit grid at the foot; page number. **Accent** none. **Silence** ~50%. **Fixed** glitch on left, Geist 400. **Free** closing line, credits. **Density** sparse.
...
- Set **Geist at 400 in cobalt**; make hierarchy with size, not weight; negative-track display.
- Track **Hanken labels uppercase 0.16em**; **Geist Mono chrome 0.04–0.08em**.
...
- Don't bold Geist, round any corner, or add a drop shadow (QR outset is an anti-shadow only).
...
- **Squint** — one Geist element dominates at 3–5× its neighbor.
...
- **Type** — Geist 400 negative-tracked, fit-to-measure; Hanken labels 0.16em; ≥1.4cqw floor.
...
- **Three Google Fonts** (Geist, Geist, Geist Mono); CJK pairing (Noto Serif SC 700/400) carries over from the source.
