...
There is a fourth state, `announce`, that the cursor never asks for: a 300×64 now-playing banner (`components/media/MediaAnnounce.tsx`) dropped in for `timing.announceMs` when music starts and retracted to the floor. It is a state rather than a timed `expanded` so it gets its own hit rect from `layout.ts` and the ordinary dwell — hovering it opens the full media card, which is where the transport controls live; the banner deliberately has none. `announce()` pins like the tray's openings and declines while the cursor is on the notch or a card is already open. Detection is `useMediaAnnounce` (app + title + artist while playing, first poll is a baseline), which is why `useMediaSession` no longer stops polling while hidden — it drops to a 2s watch rate, since hidden is exactly when this has to fire.
